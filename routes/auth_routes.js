...
router
  ...
  .post(async (req, res) => {
    ...
    try {
      ...
      firstNameInput = firstNameInput.toLowerCase().trim();
      lastNameInput = lastNameInput.toLowerCase().trim();
      emailAddressInput = emailAddressInput.toLowerCase().trim();
      firstNameInput = validation.checkString(
        firstNameInput,
        "First name"
      );
      if (firstNameInput.length < 2 || firstNameInput.length > 25) {
        throw ": Invalid first name length";
      }
      ...
      if (lastNameInput.length < 2 || lastNameInput.length > 25) {
        throw "Invalid last name length";
      }
      ...
      if (
        ...
        throw "Email address must end with stevens.edu";
      }
  ...
  .post(async (req, res) => {
    ...
    try {
      ...
      if (!emailAddressInput || !passwordInput) {
        throw "Please provide both email address and password";
      }
      emailAddressInput = emailAddressInput.toLowerCase().trim();
      if (
