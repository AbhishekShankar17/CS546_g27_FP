...
router
  ...
  //   try {
  //     // Validate input fields
  //     const { eventName, location, time, date, userFirstName, userLastName, userEmailAddress } = req.body;

  //     if (!eventName || !location || !time || !date || !userFirstName || !userLastName || !userEmailAddress) {
  //       throw "Please provide all the fields";
  //     }
  ...
  .post(async (req, res) => {
    ...
    const formattedDate = validation.formatDateForDatabase(date);

    // Check if an event with the given details exists
    const existingEvent = await getEventByDetails(eventName.toLowerCase(), location.toLowerCase(), time.toLowerCase(), formattedDate);

    if (existingEvent) {
      // Event exists, perform event registration
      await eventRegistration(eventName, location, formattedDate, time, firstName, lastName, emailAddress);
  ...
  //   try {
  //     // Validate input fields
  //     const { senderEmailAddress, receiverFirstName, receiverLastName, receiverEmailAddress, numberOfCredits } = req.body;
  ...
  //     if(typeof receiverFirstName !== "string") throw " receiverFirstName is not valid";
  //     if(typeof receiverLastName !== "string") throw " receiverLastName is not valid";
